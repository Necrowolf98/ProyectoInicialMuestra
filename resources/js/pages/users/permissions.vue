<template>
<admin>
    <v-card outlined>
        <v-card-title class="py-0 my-0" style="border-bottom: 0.1px solid #B0BEC5 ;">
            <v-col>
                <span class="headline">Usuario: {{ user.name+' '+user.lastname }}</span>
                <Link :href="route('users.show', {id: user.id})">
                    <v-btn absolute right color="primary">
                        <v-icon class="mr-2">mdi-arrow-left</v-icon>
                        Regresar
                    </v-btn>
                </Link>
            </v-col>
        </v-card-title>

        <v-card-text>
            <form>
                <v-row class="py-3">
                    <v-col cols="12" sm="12" md="6" lg="6" xl="6">
                        <v-card outlined>
                            <v-card-title class="blue lighten-1 py-1">
                                <span class="subtitle-1 text-white ml-auto mr-auto">Lista de permisos por roles de usuario</span>
                            </v-card-title>
                            <v-card-text class="pt-1">
                                <v-data-table
                                    :headers="headers"
                                    :items="arrRoles"
                                    :hide-default-footer="true"
                                    disable-pagination
                                    show-expand
                                    no-data-text="Lo sentimos, no hay nada para mostrar aquí">
                                    <template v-slot:expanded-item="{ item }">
                                        <td colspan="2" class="py-3">
                                            <th>Nombre del permiso</th>
                                            <hr>
                                            <div class="d-flex flex-column">
                                                <span v-for="(item1, index) in item.permissions" :key="index">
                                                    {{ item1.name }}
                                                </span>
                                            </div>
                                        </td>
                                        <td colspan="2" class="py-3">
                                            <th>Descripción del permiso</th>
                                            <hr>
                                            <div class="d-flex flex-column">
                                                <span v-for="(item1, index) in item.permissions" :key="index">
                                                    {{ item1.description }}
                                                </span>
                                            </div>
                                        </td>
                                    </template>
                                </v-data-table>
                            </v-card-text>
                        </v-card>
                    </v-col>

                    <v-col cols="12" sm="12" md="6" lg="6" xl="6">
                        <v-card outlined>
                            <v-card-title class="blue lighten-1 py-1">
                                <span class="subtitle-1 text-white ml-auto mr-auto">Listado de permisos</span>
                            </v-card-title>
                            <v-card-text>
                                <v-simple-table height="400px">
                                    <thead>
                                        <tr>
                                            <th>Nombre del permiso</th>
                                            <th>Descripción</th>
                                            <th>Fecha de creación</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(item, index) in ContenedorPermisos" :key="index">
                                            <td>
                                                <v-checkbox color="info"
                                                :label="item.name"
                                                :value="item"
                                                v-model="form.permissions"
                                                :value-comparator="comparatorPermissionsUserByChecked"
                                                ></v-checkbox>
                                            </td>
                                            <td v-text="item.description"></td>
                                            <td v-text="item.created_at"></td>
                                        </tr>
                                    </tbody>
                                </v-simple-table>
                            </v-card-text>
                            <v-card-actions class="grey lighten-5 py-0 my-0">
                                <v-col cols="6" class="p-0 m-0">
                                    <v-btn color="blue lighten-1" block :disabled="form.processing" :loading="form.processing" small class="text-white" @click.prevent="UpdateUserPermissions">
                                        <v-icon class="mr-1">mdi-content-save</v-icon>
                                        Actualizar
                                    </v-btn>
                                </v-col>
                                <v-col cols="6">
                                    <v-btn color="grey lighten-1" block small class="text-white" @click.prevent="CleanForm">
                                        <v-icon class="mr-1">mdi-alert</v-icon>
                                        Limpiar
                                    </v-btn>
                                </v-col>
                            </v-card-actions>
                        </v-card>
                    </v-col>
                </v-row>
            </form>
        </v-card-text>
    </v-card>
</admin>
</template>
<script>
import Admin from '../../layouts/Admin.vue';

export default {
    components: { Admin },
    
    props: ["user", "user_permission", "user_permissions_by_role", "permissions", "auth"],

    data(){
        return {
            headers: [
                {text: 'Nombre del rol', value: 'name', sortable: false},
                {text: 'Descripción del rol', value: 'description', sortable: false},
            ],

            itemId: null,
            form: this.$inertia.form({
                permissions: [],
            }),

            arrRoles: this.user.roles,
            ContenedorPermisos: this.permissions,
        }
    },

    mounted(){
        this.getPermissionsRolesByUser();
    },

    methods: {
        getPermissionsRolesByUser(){
            this.itemId = this.user.id;
            this.form.permissions = this.user.permissions;
        },

        UpdateUserPermissions(){
            this.form.put(route("users.update_permissions", this.itemId), {
                preverseScroll: true,
                onSuccess: () => {
                    this.$notify({
                        title: 'Exitoso!',
                        message: 'Se han guardado los cambios correctamente correctamente.',
                        type: 'success',
                        duration: 1500
                    });
                },
            });
        },

        comparatorPermissionsUserByChecked(a, b) {
            return a.id == b.id;
        },

        CleanForm(){
            this.form.reset();
            this.form.clearErrors();
        }
    }
}
</script>